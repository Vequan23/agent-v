...
- Use `@vraxis/agent-v/tools` for pure utilities, allowlisted HTTP, optional host browser evidence and controls, and standard approval policies.
- Use `@vraxis/agent-v/tools/node` for canonical-root discovery and file mutation, read-only Git history, and allowlisted argument-array command tools.
- Use `@vraxis/agent-v/skills` for opt-in operational skills and coding, planning, debugging, research, review, security, frontend, or document recipes. Supply product-owned instructions.
...
- Prefer the read-only planning, review, and security recipes when mutation is unnecessary. A recipe is a bounded composition, not an authority grant.
