...
import { connectMcpServer } from "../dist/adapters/mcp/index.js";
...
assert.equal(standardToolNames.gitRepositoryState, "git-repository-state");
assert.equal(standardToolNames.gitRefreshRemote, "git-refresh-remote");
assert.equal(builtInAgentSkills.repositorySync.id, "repository-sync");
...
const publicMcp = await import("@vraxis/agent-v/mcp");
...
assert.equal(publicMcp.connectMcpServer, connectMcpServer);
