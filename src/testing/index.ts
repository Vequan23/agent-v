...
export class FakeToolAgentEngine implements ToolAgentEngine {
  ...
  async run<T = string>(request: ToolAgentRequest<T>): Promise<ToolAgentResult<T>> {
    this.requests.push(request);
    const output = request.output ? request.output.parse({ ok: true }) : ("ok" as T);
    return { runId: request.runId ?? "fake-run", output, text: "ok", steps: 1, provenance: { engineId: this.descriptor.id }, durationMs: 0 };
  }
  async stream<T = string>(request: ToolAgentRequest<T>) { return { events: (async function* () {})(), result: this.run(request) }; }
}
